...
class TestLaplaceSolver2D(unittest.TestCase):
    ...
    def test_caso_trivial_jacobi(self):
        """
        Prueba el c"""Pruebas unitarias (pytest) para LaplaceSolver2D.
from campo_estatico_mdf.solver import LaplaceSolver2D




def test_trivial_zero_boundary():
s = LaplaceSolver2D(n=10, left=0, right=0, top=0, bottom=0)
V, it = s.solve_jacobi(tol=1e-8, maxiter=1000)
assert s.converged
...
def test_linear_potential_field():
# Construimos un V que depende linealmente de x: V = alpha * x
n = 20
alpha = 2.0
s = LaplaceSolver2D(n=n, left=0, right=0, top=0, bottom=0)
# crear V lineal en x (cada columna constante)
X = np.arange(n)
V_lin = np.tile(alpha * X, (n, 1)).astype(float)
s.set_potential(V_lin)
Ex, Ey = s.compute_field(spacing=1.0)
# para V = alpha * x --> dV/dx = alpha, Ex = -alpha, Ey = 0
assert np.allclose(Ex, -alpha, atol=1e-8)
assert np.allclose(Ey, 0, atol=1e-8)




def test_convergence_two_sides():
s = LaplaceSolver2D(n=30, left=0, right=10, top=0, bottom=10)
V, it = s.solve_jacobi(tol=1e-4, maxiter=5000)
assert s.converged
assert it < 5000aso trivial: todas las fronteras a 0V.
